# Accessibilité financière

Le Camp Bruchési croit que la situation financière d'une famille ne devrait jamais être un obstacle à la participation d'un enfant. Un programme de tarification différenciée est disponible pour les familles qui en ont besoin, uniquement pour le camp de séjour d'été.

---

## Programme de tarification différenciée

**Rabais disponibles : 25 % à 90 %** du tarif régulier, selon le revenu familial.

### Admissibilité

- Disponible pour le **camp de séjour d'été uniquement**
- Non applicable au camp de jour, au Camp de Noël, au Répit – Weekend, aux autres programmes saisonniers ou aux activités parascolaires

### Document requis

Le **relevé de prestations familiales** du gouvernement (avis de cotisation ou équivalent) doit être fourni pour démontrer l'admissibilité.

### Comment en bénéficier

1. Contacter le camp **avant** de s'inscrire via CampBrain
2. Fournir le relevé de prestations familiales
3. Le camp calcule le tarif applicable selon le barème en vigueur
4. L'inscription est ensuite complétée au tarif ajusté

---

## Rabais famille

Un **rabais de 50 %** s'applique automatiquement à partir du **3e enfant d'une même famille** (4e et suivants inclus), sur **tous les programmes** du camp (camp de vacances, camp de jour, programmes saisonniers).

Ce rabais s'applique lors de l'inscription via CampBrain — les enfants doivent être inscrits sous le même compte familial.

---

## Rabais employés

Les employés du camp bénéficient d'un **rabais de 20 %** pour leurs proches immédiats sur les programmes réguliers du camp.

---

## Contact

Pour toute question sur l'accessibilité financière ou pour entamer une demande :

**(450) 563-3056** · [info@campbruchesi.ca](mailto:info@campbruchesi.ca)
Lundi–Vendredi 8h–17h · Samedi–Dimanche 9h–16h
